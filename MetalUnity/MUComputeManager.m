			...
			[self.resourceManager.resources enumerateKeysAndObjectsUsingBlock:^(NSString* key, id obj, BOOL * _Nonnull stop) {
				if (obj) {
					if ([self.computeFunction containsString:@"Constant"])
					{
						...
						{
							const Float32 val = [(NSNumber*)obj floatValue];
							[encoder setBytes:&val length:(NSUInteger)sizeof(Float32) atIndex:((NSUInteger)[key integerValue])];
						}
...
-(void)dispatchMipMap
{
	if (self.pipeline)
	{
		...
		if (self.resourceManager != nil) {
			...
			MTLTextureDescriptor* texdesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA16Float width:inTex.size.width height:inTex.size.height mipmapped:TRUE];
			id<MTLTexture> mipmappedInTex = [self.context.device newTextureWithDescriptor:texdesc];
			
			MTLTextureDescriptor* newtexdesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA16Float width:outTex.size.width height:outTex.size.height mipmapped:TRUE];
